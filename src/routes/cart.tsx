...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Review your bag before checkout. 100% prepaid with quick exchange." },
    ],
  }),
  component: CartPage,
});
    ...
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-12">
      ...
      <div className="grid lg:grid-cols-[1fr_340px] gap-12 items-start">
        <div className="space-y-10">
            ...
            {items.map((i) => (
              <li key={i.key} className="py-5 flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Size {i.size}{i.custom_design_id && <span className="ml-2 text-accent font-semibold">Custom Print</span>}</p>
          ...
          {/* Store Policies Table under items */}
          <StorePoliciesNotice />
        </div>

        <aside className="bg-card border border-border p-6 h-fit lg:sticky lg:top-24 space-y-4">
          <h2 className="text-xs uppercase tracking-widest text-muted-foreground">Summary</h2>
          <div className="flex justify-between text-sm"><span>Subtotal</span><span className="font-semibold">{formatPrice(subtotal)}</span></div>
          <p className="text-xs text-muted-foreground">Shipping calculated at checkout. 100% Prepaid only (No COD).</p>
          <Link to="/checkout" className="block w-full text-center bg-accent text-accent-foreground py-4 text-sm uppercase tracking-widest font-semibold hover:bg-accent/90 transition-colors">
            Proceed to Checkout
          </Link>
